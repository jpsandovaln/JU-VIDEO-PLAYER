package ju.video.player.view.materialDesing.components.date.picker;
...
public class LightDatePicker extends DatePicker {

	public LightDatePicker(CallbackT callbackT) {
		super(callbackT);
		// TODO Auto-generated constructor stub
	}
	
	public LightDatePicker(Date currentDate, CallbackT callbackT) {
	...
	public void build() {
		...
		RoundedFillButton ready = new RoundedFillButton("", null, new Color(243, 243, 243), new Color(243, 243, 243), new Rectangle(398, 410, 80, 80), 80, 1, panel, true, new CallbackT() {
			@Override
		...
		try {
			ready.getTextLabel().setIcon(Utils.convertImageColors(Constants.RESOURCES_DIRECTORY + "/components/icons/check-icon.png", ResponsiveSwingMaterialDesign.PRIMARY_COLOR));
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		list.setFont(Utils.getMainFont(22));
			...
			public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
				...
				if (isSelected) {
					...
					if (Utils.isNumber(value.toString())) {
						c.setForeground(ResponsiveSwingMaterialDesign.PRIMARY_COLOR);
		...
		yearsBar.setBorder(new RoundedBorder(30, new Color(243, 243, 243), true));
		...
		CallbackT callbackT = new CallbackT() {
			...
			public void done(Object s) {
				...
				for (RoundedFillButton roundedButton : buttons) {
					if (roundedButton.getTextLabel().getForeground().equals(Color.WHITE)) {
						last = roundedButton;
					}
				...
				next.getTextLabel().setForeground(Color.WHITE);
				next.getTextLabel().setFont(Utils.getBoldFont(22));

				if (last != null) {
					last.getBackgroundButton().recolor(Color.WHITE, 50, true);
					last.getTextLabel().setForeground(Color.BLACK);
		...
		for (int l = 0; l < 6; l++) {
			for (int c = 0; c < 7; c++) {
				...
				if (s.equalsIgnoreCase(String.valueOf(currentDay))) {
					button = new RoundedFillButton(s, Utils.getBoldFont(22), Color.WHITE, ResponsiveSwingMaterialDesign.PRIMARY_COLOR, new Rectangle(x, y, 50, 50), 50, 1, panel, true, callbackT);
				} else {
					button = new RoundedFillButton(s, Utils.getMainFont(22), Color.BLACK, Color.WHITE, new Rectangle(x, y, 50, 50), 50, 1, panel, true, callbackT);
				}
